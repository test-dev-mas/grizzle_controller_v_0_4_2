...
void test_5() {
    ...
    (led_blink == 18 || led_blink == 19) ? (test_result[0] = 0) : (test_result[0] = 1);
...
void test_6() {
    ...
    // DDRK |= (1 << relay_16[10].pin) | (1 << relay_16[9].pin);
    // _delay_ms(500);
    // DDRC |= (1 << relay_16[2].pin);
    // _delay_ms(500);
    // DDRC |= (1 << relay_16[0].pin) | (1 << relay_16[1].pin);
    ...
    */
    ...
    for (;;) {
        ...
        if (rms_count == 10) {                                      // calculate RMS at 10Hz
            ...
            if ((rms_diff > 20) && (!positive_edge_detected)) {
                enable_beep();
                // uart0_transmit('+');
        ...
        if (beep_flag) {
            ...
            case 1:                                                 // falling edge
                if (tick - t_1 < 720 && tick - t_1 > 622) {         // beep ON width: 655 ms +/- 5%
                    number_beep++;
                }
                edge_direction = 0;
                break;
...
void test_7() {
    ...
    (led_blink == 8 || led_blink == 9) ? (test_result[0] = 0) : (test_result[0] = 1);
...
void test_8() {
    ...
    (led_blink == 4 || led_blink == 5) ? (test_result[0] = 0) : (test_result[0] = 1);
